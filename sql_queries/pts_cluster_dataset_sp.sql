CREATE OR ALTER PROCEDURE seasonPtsDataSetByCluster
    @pts_thresh FLOAT,
    @cluster_id INT,
	@line_type varchar(max),
	@cluster_type varchar(max)
...
SELECT 
  ...
  WHERE  pc.'+@cluster_type+N'Cluster = @playerIdParam and   pgl.yearSeason in (2024,2023)
 ...
 where b.yearSeason in (2024)
  ...
  N'@playerIdParam INT, @ptsThreshParam FLOAT, @cluster_typeParam varchar(max)', 
                       @playerIdParam = @cluster_id, 
                       @ptsThreshParam = @pts_thresh,
					   @cluster_typeParam = @cluster_type;
  end;
